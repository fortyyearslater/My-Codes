...
int hd[max_n], des[max_n], nxt[max_n], val[max_n], oud[max_n], e_cnt = 0;
ll rans = 0;
...
void dfs(int id, int fa, ll cur)
{
	rans += cur;
	for (int p = hd[id]; p != -1; p = nxt[p])
		if (des[p] != fa)
			dfs(des[p], id, cur + val[p]);
}

signed main(int argc, char *argv[])
{
	setName("problem corridor");
	registerTestlibCmd(argc, argv);
	
	string js = ans.readLine(), ps = ouf.readLine();
	if (js != ps)
		quitf(_wa, "A %s %s", js.c_str(), ps.c_str());
	
	string mes = ouf.readLine();
	
	int n = inf.readInt(), s = inf.readInt() - 1, t = inf.readInt() - 1;
	// inf.readEoln();
	...
	for (int i = 1, x, y, c; i < n; i++)
	{
		...
		if (mes[i-1] == '1')
			...
			quitp(0.6, "C \'%c\'", mes[i-1]);
	}
	
	for (int i = 0; i < n; i++)
		if (oud[i] > 1 || !((i == s || i == t) ^ oud[i]))
			quitp(0.6, "D");
	
	dfs(s, -1, 0), dfs(t, -1, 0);
	if (rans != atoll(js.c_str()))
		quitp(0.6, "E %s %lld", js.c_str(), rans);
	else
		quitf(_ok, "");
